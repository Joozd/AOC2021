...
class PNGMap<T>(private val map: List<CoordinateWithValue<T>>, private val scale: Int = 5, private var definedColors: ColorMaker<T>? = null) {
    ...
    private fun draw(image: BufferedImage, dimensions: DrawingArea){
        map.forEach {
            ...
            val color = definedColors?.getColor(it) ?: 0xFFFFFF
    ...
    private class DrawingArea(map: List<CoordinateWithValue<*>>){
    ...
    fun interface ColorMaker<T>{
        fun getColor(value: CoordinateWithValue<T>): Int
    }
}